...
pub fn react_circle_arc(
    ...
    if react.0.is_sign_negative() {
        let true_start = Angle(std::f64::consts::PI / 2.0 + start.0);
        let length = Angle(end.0 - start.0);
        CircleArc {
            circle: react_circle,
            start: true_start,
            length,
...
pub fn res_circle_arc(
    ...
    let true_start = if react_start.0 != 0.0 {
        ...
        if react_start.0.is_sign_negative() {
            start
        } else {
    ...
    let end = if react_end.0 != 0.0 {
        if react_end.0.is_sign_negative() {
            Angle(end.0)
        } else {
    ...
    let length = Angle(end.0 - true_start.0);
